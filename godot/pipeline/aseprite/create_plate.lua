...
sprite.filename = "plate.aseprite"
...
local function drawRect(cel, x, y, w, h, color)
    ...
    -- Top/Bottom
    for px = x, x + w - 1 do
        if y >= 0 and y < img.height then img:drawPixel(px, y, color) end
        if y + h - 1 >= 0 and y + h - 1 < img.height then img:drawPixel(px, y + h - 1, color) end
    end
    -- Left/Right
    for py = y, y + h - 1 do
        if x >= 0 and x < img.width then img:drawPixel(x, py, color) end
        if x + w - 1 >= 0 and x + w - 1 < img.width then img:drawPixel(x + w - 1, py, color) end
    end
end
